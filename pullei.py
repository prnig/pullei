...
def fetch_pull_requests(github_token):
    ...
    while True:
        ...
        for pull_request in prs_data:
            pr_date = datetime.strptime(pull_request['created_at'], '%Y-%m-%dT%H:%M:%SZ').strftime('%d-%m-%y')
            pull_requests.append((pull_request['number'], pr_date))
...
def download_file(url, nuclei_templates_dir, cves_only, download_directory, pr_date):
    decoded_url = unquote(url)
    file_name = os.path.basename(decoded_url)

    download_path = os.path.join(download_directory, "pullei")
    if "cves" in url:
        download_path = os.path.join(download_path, "cves", pr_date)
    else:
        download_path = os.path.join(download_path, "other", pr_date)
...
def execute_script():
    ...
    for pull_request_number, pr_date in pull_request_numbers:
        file_urls = fetch_files(pull_request_number, script_args.github_token)
        for url in file_urls:
            download_file(url, script_args.nuclei_templates_path, script_args.cves_only, script_args.download_directory, pr_date)
